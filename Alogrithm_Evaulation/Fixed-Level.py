...
def random_placement_algorithm_dram(rectangles, strip_width_dram, m):
    ...
    for rect in rectangles:
        ...
        for i, level in enumerate(levels):
            ...
            if level_width + rect[1] <= strip_width_dram and len(level) < m and rect[2] <= level_heights[i]: 
                level.append(rect) 
                placed = True
                break
        ...
        if not placed:
            levels.append([rect]) 
            level_heights.append(rect[2])  

    dram_packing = []
    current_height = 0
    for level, height_limit in zip(levels, level_heights):
        level_width = 0
        for rect in level:
