...
def ffdh_algorithm_dram(rectangles, strip_width_dram, m):
    ...
    for rect in rectangles:
        ...
        for i, level in enumerate(dram_levels):
            if len(level) < m:
                ...
                if level_width + rect[1] <= strip_width_dram:
                    level.append(rect)
                    placed = True
                    break
        if not placed:
...
def ffdh_algorithm_pmem(rectangles, temp_dram_packing, strip_width, m, max_height, step=0.1):
...
if __name__ == "__main__":
    ...
    dram_packing, dram_max_ceiling = ffdh_algorithm_dram(rectangles, strip_width_dram, m_value)
    pmem_max_ceiling = 0
    pmem_packing=[]
    ...
    while True:
        ...
        pmem_packing, pmem_max_ceiling = ffdh_algorithm_pmem(pmem_rectangles, temp_dram_packing, strip_width_pmem, m_value, dram_max_ceiling)
