...
# 讀取 Excel 檔案
df = pd.read_excel('fig1.xlsx')

# 從 excel 中提取數據
task = df['Task Amount'].tolist()   #對應excel的Task Amount
...
g = gnuplot.Gnuplot()
g('set terminal png size 800,600 font "Arial,16"')  # 設定輸出格式和大小，並設置字體大小
g('set output "fig1.png"')          # 設定輸出檔名為 fig1.png
#g('set title "Comparisons of Algorithms Across Different Task Amounts" font "Arial,16"')  # 設定圖表標題
g('set xlabel "Number of Tasks" font "Arial,18"')       # 設定 x 軸標籤
g('set ylabel "Completion Time (seconds)" font "Arial,18"')  # 設定 y 軸標籤
g('set grid')                      # 啟用網格
g('set key font "Arial,16"')        # 設定圖例字體大小
